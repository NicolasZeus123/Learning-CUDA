...
/**
 * @brief 计算矩阵的迹（对角线元素之和）的GPU内核函数
 * 
 * 矩阵的迹定义为对角线元素之和。此函数期望一个按行优先展开的矩阵。
 * 如果矩阵不是方阵，迹将沿主对角线求和，最多到行数和列数中的较小值。
 * 
 * @tparam T 矩阵元素的数据类型（如 float、int）
 * @param d_input 展平的矩阵，大小为 rows * cols
 * @param d_partial_sum 用于存储中间计算结果的数组
 * @param rows 矩阵的行数
 * @param cols 矩阵的列数
 */
template <typename T>
__global__ void trace_kernel(const T* d_input, T* d_partial_sum, size_t rows, size_t cols) {
  extern __shared__ char shared_mem[];  // 共享内存，大小在启动内核时指定
  ...
  // 将块的结果写回全局内存
  if (tid == 0) {
    d_partial_sum[blockIdx.x] = sdata[0];
  }
}
 ...
 * 
 * @tparam T 矩阵元素的数据类型（如 float、int）
 * @param h_input 展平的矩阵，大小为 rows * cols
 * @param rows 矩阵的行数
 * @param cols 矩阵的列数
 * @return 矩阵的迹（对角线元素之和）
 */
...
T trace(const std::vector<T>& h_input, size_t rows, size_t cols) {
  ...
  // 分配部分求和结果的内存
  cudaMalloc(&d_partial_sum, grid_size * sizeof(T));

  // 启动内核
  const size_t shared_mem_size = block_size * sizeof(T);
  trace_kernel<<<grid_size, block_size, shared_mem_size>>>(
      d_input, d_partial_sum, rows, cols);

  // 复制部分求和结果回主机
  std::vector<T> h_partial_sum(grid_size);
  cudaDeviceSynchronize();
  cudaMemcpy(h_partial_sum.data(), d_partial_sum, grid_size * sizeof(T), cudaMemcpyDeviceToHost);

  // 最终求和
  T total = T{0};
  for (T val : h_partial_sum) total += val;
 ...
 * @brief 将任意类型转换为float
 * 
 * 支持half、float、double等类型。如果是half类型，使用__half2float转换。
 * 
 * @tparam T 输入数据类型
 * @param val 输入值
 * @return 转换后的float值
...
/**
 * @brief 执行FlashAttention前向计算，支持分组查询注意力（GQA）和因果掩码
 * 
 * 所有张量必须为连续内存（contiguous），按行优先（C-order）展开。
 * blockDim.x = Br * 32
 * 
 * @tparam T 输入/输出张量的数据类型（如float或__half）
 * @tparam Br 每次处理的Query分块行数（目标序列方向的tile高度）
 * @tparam Bc 每次加载的Key/Value分块行数（源序列方向的tile高度）
 * 
 * @param[in] q Query张量，形状 [batch_size, tgt_seq_len, query_heads, head_dim]
 * @param[in] k Key张量，形状 [batch_size, src_seq_len, kv_heads, head_dim]
 * @param[in] v Value张量，形状 [batch_size, src_seq_len, kv_heads, head_dim]
 * @param[out] o 输出张量，形状 [batch_size, tgt_seq_len, query_heads, head_dim]
 * @param[in,out] d_m Softmax最大值缓冲区，形状 [batch_size, query_heads, tgt_seq_len]
 * @param[in,out] d_l Softmax归一化因子缓冲区，形状 [batch_size, query_heads, tgt_seq_len]
 * @param[in] batch_size 批次大小
 * @param[in] target_seq_len 目标序列长度（Query序列长度）
 * @param[in] src_seq_len 源序列长度（Key/Value序列长度）
 * @param[in] query_heads Query头的数量
 * @param[in] kv_heads Key/Value头的数量（必须整除query_heads）
 * @param[in] head_dim 每个注意力头的维度
 * @param[in] is_causal 是否启用因果掩码（true表示仅允许j <= i的注意力）
 */
...
#pragma unroll
  for (int idx = threadIdx.x; idx < Br * head_dim; idx += blockDim.x) {
    ...
    if (global_q < target_seq_len) {
      ...
      val = d_q[q_offset * head_dim + d_idx];
    }
    s_Q[idx] = val;
  }
  __syncthreads();
  ...
  if (valid_query) {
    ...
    row_m_prev = d_m[ml_offset];
    row_l_prev = d_l[ml_offset];
  }
...
#pragma unroll
    for (int idx = threadIdx.x; idx < Bc * head_dim; idx += blockDim.x) {
      ...
      if (global_k < src_seq_len) {
        ...
        k_val = d_k[kv_offset * head_dim + d_idx];
        v_val = d_v[kv_offset * head_dim + d_idx];
      }
      s_K[idx] = k_val;
      s_V[idx] = v_val;
    }
    ...
    // 6.2 计算局部logits和最大值（第一遍）
    double local_m = -__DBL_MAX__;
    double local_p[Bc];  // logits或权重
...
#pragma unroll
        for (int local_k = 0; local_k < actual_Bc; ++local_k) {
          ...
          pv_sum += local_p[local_k] *
                    static_cast<double>(to_float(s_V[local_k * head_dim + d]));
        }
  ...
  if (valid_query) {
    // 写回m/l
    const int64_t ml_offset = ((int64_t)batch_id * query_heads + q_head_id) * target_seq_len + q_pos;
    d_m[ml_offset] = row_m_prev;
    d_l[ml_offset] = row_l_prev;

    // 写回输出O
    const int64_t o_offset = ((int64_t)batch_id * target_seq_len + q_pos) * query_heads + q_head_id;
#pragma unroll
    for (int d = 0; d < head_dim; ++d) {
      d_o[o_offset * head_dim + d] = from_float<T>(static_cast<float>(acc_o[d]));
    }
  }
}

/**
 * @brief 计算FlashAttention
 * 
 * 对给定的query、key、value张量执行注意力计算，支持因果掩码和分组查询注意力。
 * 
 * @tparam T 输入/输出张量的数据类型（如float、half）
 * @param[in] h_q Query张量，形状 [batch_size, tgt_seq_len, query_heads, head_dim]
 * @param[in] h_k Key张量，形状 [batch_size, src_seq_len, kv_heads, head_dim]
 * @param[in] h_v Value张量，形状 [batch_size, src_seq_len, kv_heads, head_dim]
 * @param[out] h_o 输出注意力张量，形状 [batch_size, tgt_seq_len, query_heads, head_dim]
 * @param[in] batch_size 批次维度大小
 * @param[in] target_seq_len 目标序列长度
 * @param[in] src_seq_len 源序列长度
 * @param[in] query_heads Query注意力头的数量
 * @param[in] kv_heads Key/Value头的数量（支持分组查询注意力）
 * @param[in] head_dim 每个注意力头的维度
 * @param[in] is_causal 是否应用因果掩码
 */
template <typename T>
void flashAttention(const std::vector<T>& h_q,
  ...
  if (query_heads % kv_heads != 0) {
    throw std::invalid_argument("query_heads必须能被kv_heads整除");
  }
  ...
  // Grid:
  //   x: query分块数 = ceil(target_seq_len / Br)
  //   y: (batch, query_head)对数 = batch_size * query_heads
  dim3 grid((target_seq_len + Br - 1) / Br, batch_size * query_heads);

  // Block: Br个warp → Br * 32个线程
  dim3 block(Br * 32);

  // 共享内存: s_Q[Br*head_dim] + s_K[Bc*head_dim] + s_V[Bc*head_dim]
  size_t shared_mem_size = (Br + 2 * Bc) * head_dim * sizeof(T);

  // 7. 启动内核
  flashAttention_<T, Br, Bc><<<grid, block, shared_mem_size>>>(
      d_q, d_k, d_v, d_o, d_m, d_l, batch_size, target_seq_len, src_seq_len,
...
template void flashAttention<float>(const std::vector<float>&, const std::vector<float>&,
                                    const std::vector<float>&, std::vector<float>&, int, int, int,
                                    int, int, int, bool);
template void flashAttention<half>(const std::vector<half>&, const std::vector<half>&,
                                   const std::vector<half>&, std::vector<half>&, int, int, int, int,
                                   int, int, bool);